...
describe("model role catalog", () => {
  it("resolves current xAI defaults per role", () => {
    ...
    expect(getModelId("bulk")).toBe("grok-4.1-fast-non-reasoning");
  ...
  it("honours per-role env overrides", () => {
    ...
    expect(getModelId("bulk")).toBe("grok-4-fast-non-reasoning");
    expect(getModelId("vision")).toBe("grok-4.3");
    expect(getImageModelId()).toBe("grok-imagine-image");
  });

  it("remaps the grok-4.20 non-reasoning family onto the live Gateway slug", () => {
    vi.stubEnv("AI_PROVIDER", "xai");
    ...
    expect(getModelId("bulk")).toBe("spacexai/grok-4.1-fast-non-reasoning");
  });
...
describe("isRetiredBulkSlug", () => {
  it("matches the grok-4.20 non-reasoning family, including gateway prefixes", () => {
    ...
    expect(isRetiredBulkSlug("grok-4.1-fast-non-reasoning")).toBe(false);
