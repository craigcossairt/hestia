...
describe("model role catalog", () => {
  ...
  it("honours per-role env overrides", () => {
    vi.stubEnv("AI_PROVIDER", "xai");
    vi.stubEnv("AI_MODEL_BULK", "grok-4.20-non-reasoning-latest");
    vi.stubEnv("AI_MODEL_VISION", "grok-4.3");
    vi.stubEnv("AI_MODEL_IMAGE", "grok-imagine-image");

    expect(getModelId("bulk")).toBe("grok-4.20-non-reasoning-latest");
    expect(getModelId("vision")).toBe("grok-4.3");
    expect(getImageModelId()).toBe("grok-imagine-image");
  });

  it("remaps the dated 4.20 snapshot env override onto the current alias", () => {
    vi.stubEnv("AI_PROVIDER", "xai");
    vi.stubEnv("AI_MODEL_BULK", "grok-4.20-0309-non-reasoning");
    expect(getModelId("bulk")).toBe("grok-4.20-non-reasoning");
    vi.stubEnv("AI_PROVIDER", "gateway");
    vi.stubEnv("AI_MODEL_BULK", "xai/grok-4.20-0309-non-reasoning");
    expect(getModelId("bulk")).toBe("xai/grok-4.20-non-reasoning");
  });
...
describe("isRetiredBulkSlug", () => {
  it("matches the dated 4.20 snapshot, including gateway prefixes", () => {
    expect(isRetiredBulkSlug("grok-4.20-0309-non-reasoning")).toBe(true);
    expect(isRetiredBulkSlug("xai/grok-4.20-0309-non-reasoning")).toBe(true);
    expect(isRetiredBulkSlug("grok-4.20-non-reasoning")).toBe(false);
